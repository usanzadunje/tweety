<div class="flex">
    ...
    <div class="flex items-center mr-4 {{ $tweet->isLikedBy(current_user()) ? 'text-blue-700' : 'text-gray-700' }}">
    ...
    <div class="flex items-center {{ $tweet->isDislikedBy(current_user()) ? 'text-red-700' : 'text-gray-700' }}">
        <button wire:click="dislike" class="text-xs flex items-center">
            <svg viewBox="0 0 20 20" class="mr-1 w-3">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
